...
class HIDPollingThread(QThread):
    ...
    def run(self):
        ...
        while self.running:
            ...
            # Sleep 1ms to prevent exactly zero 100% CPU lock, but still effectively 1000hz poll
            time.sleep(0.001)
...
class SmartRecorder(QObject):
    ...
    def on_packet(self, packet):
        ...
        # 3. Emit Real-time State for UI Cursor (Even when not recording)
...
class ModernCalculationEngine:
    ...
    def calculate(data_points, max_x, max_y, phys_w_mm, phys_h_mm, aspect_ratio=None):
        ...
        # Safety clamp maxes avoiding div by 0
    ...
    def update_state(self, nx, ny, is_down, max_x, max_y):
        ...
        if is_down:
            ...
            if len(self.current_stroke) > 1:
                ...
                self.scene.addLine(p1[0], p1[1], p2[0], p2[1], trace_pen)
...
class ApplicationWindow(QMainWindow):
    def __init__(self):
        ...
        self.hid_thread.start() # Background infinite poll
        
        self.recorder = SmartRecorder(self.tablet_info)
        
        # Route HID packets -> Smart Recorder
    ...
    def _connect_hw(self):
        if self.hid_thread.open_device():
...
if __name__ == '__main__':
    # Force Qt to completely ignore all pen and touch APIs
    sys.argv.extend(["-plugin", "platforms/qwindows:nowmpointer"])
